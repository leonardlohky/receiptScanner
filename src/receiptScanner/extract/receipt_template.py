...
"""
This module abstracts templates for receipt providers.
...
PARSERS_MAPPING = {"lines": parsers.lines, "regex": parsers.regex}
...
class InvoiceTemplate(OrderedDict):
    ...
    def extract(self, optimized_str):
        ...
        if "required_fields" not in self.keys():
            required_fields = ["date", "amount", "receipt_number", "issuer"]
        else:
            required_fields = []
        ...
        if set(required_fields).issubset(output.keys()):
            output["desc"] = "Receipt from %s" % (self["issuer"])
            logger.debug(output)
            return output
        else:
